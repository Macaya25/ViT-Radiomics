#https://github.com/MathieuNlp/Sam_LoRA/tree/main
import numpy as np
import torch
...
class convLoRA_qkv(nn.Module):
    ...
    def __init__(
        ...
        super(convLoRA_qkv, self).__init__()
        self.qkv = qkv
        ...
        self.lora_moe_gating_q = lora_moe_gating_q
        self.lora_moe_gating_v = lora_moe_gating_v
        self.lora_moe_experts_q = lora_moe_experts_q
        self.lora_moe_experts_v = lora_moe_experts_v
    ...
    def forward(self, x: Tensor):
        ...
        lora_res_q = self.linear_a_q(x)
        lora_res_v = self.linear_a_v(x)

        #moe_q
        dim = lora_res_q.dim()
        if dim == 3:
            B, L, C = lora_res_q.size()
            H = W = int(math.sqrt(L))
            lora_res_q = lora_res_q.reshape(B, H, W, C)
        else:
            H, W = lora_res_q.size()[1:3]

        # Calculate the gating values.
        lora_res_q = lora_res_q.permute(0, 3, 1, 2).contiguous()
        gates_q, moe_loss_q = self.lora_moe_gating_q(lora_res_q)

        # Distribute data samples to experts.
        dispatcher = SparseDispatcher(self.num_experts, gates_q)
        expert_inputs = dispatcher.dispatch(lora_res_q)
        expert_outputs = []
        for i in range(self.num_experts):
            ...
            cur_res = self.lora_moe_experts_q[i](cur_res)
            if upsample_ratio != 1:
        ...
        temp_lora_res_q = dispatcher.combine(expert_outputs, multiply_by_gates=self.multiply_by_gates)
        lora_res_q = lora_res_q + temp_lora_res_q

        lora_res_q = lora_res_q.permute(0, 2, 3, 1).contiguous()
        if dim == 3:
            lora_res_q = lora_res_q.reshape(B, L, C)

        #moe_v
        dim = lora_res_v.dim()
        if dim == 3:
            B, L, C = lora_res_v.size()
            H = W = int(math.sqrt(L))
            lora_res_v = lora_res_v.reshape(B, H, W, C)
        else:
            H, W = lora_res_v.size()[1:3]

        # Calculate the gating values.
        lora_res_v = lora_res_v.permute(0, 3, 1, 2).contiguous()
        gates_v, moe_loss_v = self.lora_moe_gating_v(lora_res_v)

        # Distribute data samples to experts.
        dispatcher = SparseDispatcher(self.num_experts, gates_v)
        expert_inputs = dispatcher.dispatch(lora_res_v)
        expert_outputs = []
        for i in range(self.num_experts):
            if len(expert_inputs[i]) == 0:
                continue
            ...
            if upsample_ratio != 1:
                cur_res = F.interpolate(cur_res, scale_factor=upsample_ratio, mode="bicubic")
            cur_res = self.lora_moe_experts_v[i](cur_res)
            if upsample_ratio != 1:
                cur_res = F.interpolate(cur_res, size=(int(H), int(W)), mode="bicubic")
            expert_outputs.append(cur_res)

        # Combine data samples after processing by each expert.
        temp_lora_res_v = dispatcher.combine(expert_outputs, multiply_by_gates=self.multiply_by_gates)
        lora_res_v = lora_res_v + temp_lora_res_v

        lora_res_v = lora_res_v.permute(0, 2, 3, 1).contiguous()
        if dim == 3:
            lora_res_v = lora_res_v.reshape(B, L, C)

        q_ba = self.linear_b_q(lora_res_q)
        v_ba = self.linear_b_v(lora_res_v)
        qkv[:, :, :, :self.d_model] += q_ba #q part
        qkv[:, :, :, -self.d_model:] += v_ba #v part



        return qkv #, moe_loss_v, moe_loss_q
    
class convLoRA_sam(nn.Module):
    """
    ...
    def __init__(self, sam_model, rank: int, lora_layer=None):
        super(convLoRA_sam, self).__init__()
        self.rank = rank
        assert rank > 0
        ...
        for t_layer_i, blk in enumerate(sam_model.image_encoder.blocks):
            ...
            lora_moe_gating_q = MoEGate(M=conv_lora_expert_num, d=self.rank, K=topk)
            lora_moe_gating_v = MoEGate(M=conv_lora_expert_num, d=self.rank, K=topk)
            lora_moe_experts_q = nn.ModuleList([])
            lora_moe_experts_v = nn.ModuleList([])
            upsample_ratios = list(range(1, conv_lora_expert_num + 1))

            for upsample_ratio in upsample_ratios:
                expert = nn.Conv2d(in_channels=self.rank, out_channels=self.rank, kernel_size=3, stride=1, padding=1, bias=True)
                expert.bias.data.zero_()
                lora_moe_experts_q.append(nn.Sequential(expert, nn.GELU()))
            
            for upsample_ratio in upsample_ratios:
                expert = nn.Conv2d(in_channels=self.rank, out_channels=self.rank, kernel_size=3, stride=1, padding=1, bias=True)
                expert.bias.data.zero_()
                lora_moe_experts_v.append(nn.Sequential(expert, nn.GELU()))

            num_experts = conv_lora_expert_num
            multiply_by_gates = False
...
#https://github.com/autogluon/autogluon/blob/master/multimodal/src/autogluon/multimodal/models/adaptation_layers.py#L606
